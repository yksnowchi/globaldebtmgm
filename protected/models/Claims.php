...
class Claims extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('file_number, type_of_debt', 'length', 'max'=>255),
			array('settlement_date, updated_at', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, Fk_clientid, Fk_creditor_id, file_number, type_of_debt, amount_of_claim, days_behind, settlement_date, settlement_amount, savings, created_at, updated_at', 'safe', 'on'=>'search'),
		);
	}
